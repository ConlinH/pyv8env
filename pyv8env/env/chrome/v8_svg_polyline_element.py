...
class SVGPolylineElement(SVGGeometryElement):
    ...
    def __init__(self, *args, **kw):
        super(SVGPolylineElement, self).__init__(*args, **kw)
    ...
    def get_points(self):
        ...
        if val is not None:
            return val
        logger.debug(f'patch -> v8_svg_polyline_element.py -> SVGPolylineElement.points -> get attr')

    def get_animatedPoints(self):
        val = self._attr.get('animatedPoints')
        if val is not None:
            return val
        logger.debug(f'patch -> v8_svg_polyline_element.py -> SVGPolylineElement.animatedPoints -> get attr')

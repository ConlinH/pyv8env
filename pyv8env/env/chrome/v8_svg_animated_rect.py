...
class SVGAnimatedRect:
    ...
    def get_baseVal(self):
        ...
        if val is not None:
            return val
        logger.debug(f'patch -> v8_svg_animated_rect.py -> SVGAnimatedRect.baseVal -> get attr')

    def get_animVal(self):
        val = self._attr.get('animVal')
        if val is not None:
            return val
        logger.debug(f'patch -> v8_svg_animated_rect.py -> SVGAnimatedRect.animVal -> get attr')

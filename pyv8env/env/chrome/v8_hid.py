...
class HID(EventTarget):
    ...
    def __init__(self, *args, **kw):
        super(HID, self).__init__(*args, **kw)
    ...
    def get_onconnect(self):
        ...
        if val is not None:
            return val
        logger.debug(f'patch -> v8_hid.py -> HID.onconnect -> get attr')
    ...
    def get_ondisconnect(self):
        ...
        if val is not None:
            return val
        logger.debug(f'patch -> v8_hid.py -> HID.ondisconnect -> get attr')

    def set_ondisconnect(self, val):
        self._attr['ondisconnect'] = val

    def fn_getDevices(self, *args):
        logger.debug(f'patch -> v8_hid.py -> HID.getDevices{tuple(args)} -> method')

    def fn_requestDevice(self, *args):
        logger.debug(f'patch -> v8_hid.py -> HID.requestDevice{tuple(args)} -> method')
